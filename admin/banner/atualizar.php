<?php
    ...
    if(isset($_POST['altBanner'])){
        ...
        $textBanner = $_POST['textBanner'];
        $linkBanner = $_POST['linkBanner'];
        $statusBanner = $_POST['statusBanner'];

        if(!empty($_FILES['imgBanner']['name'])){ // se nõa estiver vazia o campo do file
            ...
            if(move_uploaded_file($arquivo['tmp_name'], '../img/banner/' . $arquivo['name'] )){ //salvar na pasta
                $imgBanner = 'banner/' . $arquivo['name']; // gravar no banco de dados
            }else{
...
<div class="inserir-container">
    ...
    <form class="form-container" action="index.php?p=banner&b=banner&id=<?php echo $banner->idBanner?>" method="POST" enctype="multipart/form-data" class="form">
        ...
        <div class="form-img">
            ...
            <input type="file" name="imgBanner" id="inputImagem">
            </div>
        </div>

        <div class="form-content">
            <input required type="text" id="altBanner" name="altBanner" placeholder="Informe o texto alternativo: " value="<?php echo $banner->altBanner?>">
            <textarea required name="textBanner" id="textBanner" cols="30" rows="10" placeholder="Informe o texto: "><?php echo $banner->textBanner?></textarea>
            <input required type="url" name="linkBanner" id="linkBanner" placeholder="Informe o link: " value="<?php echo $banner->linkBanner?>">
            <input class="formBtn" type="submit" value="Atualizar">
        </div>

        <div>
            
        <input required type="checkbox" name="statusBanner" id="checkbox" value="<?php echo $banner->statusBanner?>">
